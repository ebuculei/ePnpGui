...
namespace ePnpGui
{
    public partial class PnpGuiForm
    {
        ...
        private void AddDriverPackage2(string infName, bool install)
        {
            if (!this.backgroundWorker1.IsBusy)
            {
                this.CleanupContext(this.context);
                this.context.Code = install ? OperationCode.AddInstallDriver : OperationCode.AddDriver;
        ...
        private void DeleteDriverPackages2(List<DriverStoreEntry> ldse, bool force)
        {
            if (!this.backgroundWorker1.IsBusy)
            {
                this.CleanupContext(this.context);
                this.context.Code = force ? OperationCode.ForceDeleteDriver : OperationCode.DeleteDriver;
